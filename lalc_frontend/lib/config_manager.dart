...
class ConfigManager {
  ...
  Future<void> _checkAndExtractDefaultConfig() async {
    ...
    try {
      logger.d('未找到luyaoqi_config，开始从assets解压...');
      ...
      // 解压到configs目录
      await ZipHelper.unzipTo(zipFile: luyaoqiConfigZip.path, dstDir: luyaoqiConfigDir.path);
      
      // 删除ZIP文件，只保留解压后的目录
      await luyaoqiConfigZip.delete();
      
      logger.d('luyaoqi_config解压成功');
    } catch (e) {
      logger.e('解压luyaoqi_config失败: $e');
